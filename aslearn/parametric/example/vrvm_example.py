...
sw1 = np.sign(np.sin(2*np.pi*0.8*X))
sw2 = np.sign(np.cos(2*np.pi*0.8*X))
sw3 = np.sign(np.sin(2*np.pi*1.2*X))
Y = np.concatenate([sw1, sw2, sw3], axis=1) + np.random.randn(300,3) * 0.1

poly = PolynomialFT(degree=3)
fri = FourierFT(degree=np.linspace(0.1,10,7))
X_f = poly(fri(X))
