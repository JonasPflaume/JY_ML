import torch as th
from jylearn.parametric.classification import Classification
    ...
    def fit(self, X, Y):
        ...
        while not stop_flag:
            ...
            for j in range(self.classNum):
                for k in range(self.classNum):
                    ...
                    if k == j:
                        ...
                        hessian_block = Y_pred[:,k] * (0 - Y_pred[:,j])
                        hessian_block = th.einsum("b,bij->ij", hessian_block, feature_outer_product)
            ...
            try:
                ...
                # pinv is numerically more stable. We should use line search here... whatever
            except:
...
if __name__ == "__main__":
    from jylearn.data.get_data import classification_toy_data
    ...
    ### sloppy illustraion of decision boundary
